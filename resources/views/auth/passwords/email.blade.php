                    ...
                    <form role="form" action="{{ route('password.email') }}" method="post">
                        ...
                        <fieldset>
                            <div class="form-group">
                                <input class="form-control @error('email') is-invalid @enderror" placeholder="البريد الإلكتروني" name="email" value="{{ old('email') }}" type="email" autocomplete="off" required>
                                @error('email')
                                <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                @enderror
                            </div>
                            <button type="submit" class="btn btn-lg btn-success btn-block mb-3">أرسل الرابط</button>
                        </fieldset>
                    </form>

                    <a class="btn btn-link" href="{{ route('login') }}">صفحة تسجيل الدخول</a>
